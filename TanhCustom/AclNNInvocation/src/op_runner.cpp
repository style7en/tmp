...
bool OpRunner::RunOp()
{
    ...
    auto ret = aclnnTanhCustomGetWorkspaceSize(inputTensor_[0], outputTensor_[0],
    ...
    //添加执行算子代码
    if (aclnnTanhCustom(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
